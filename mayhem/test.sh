#!/usr/bin/env bash
#
# isort/mayhem/test.sh — RUN isort's own pytest known-answer suite and emit a CTRF summary.
# exit 0 iff no test failed.
#
# PATCH-grade behavioral oracle: the selected tests (tests/unit/test_isort.py, test_regressions.py,
# test_ticketed_features.py, test_api.py, test_exceptions.py) are thousands of parametrized
# known-answer cases asserting isort.code(...) / isort.check_code(...) / api.sort_file(...) output
# EXACTLY. A no-op / "exit(0)" / behavior-altering patch to isort cannot pass it.
#
# The suite is run through the `/mayhem/isort-tests` ELF launcher (which exec()s
...
# executable. The system `python3` under /usr/bin is SPARED, but `/mayhem/isort-tests` is not —
...
if [ ! -x /mayhem/isort-tests ]; then
  echo "FATAL: /mayhem/isort-tests launcher missing — run mayhem/build.sh first" >&2
  emit_ctrf "pytest" 0 1 0
  exit 2
fi

echo "=== running pytest (isort known-answer suite) via /mayhem/isort-tests ==="
out="$(/mayhem/isort-tests 2>&1)"; rc=$?
echo "$out"
